...
public class ExportPdf extends ExportVectorGraphic {
	...
	public void startupMapHook() {
		...
		HashMap<Key, Float> transcodingHints = null;
		...
		try {
			for (Iterator<File> it = documentsToOpen.iterator(); it.hasNext();) {
				File fileToOpen = it.next();
	 ...
	 */
	public HashMap<Key, Float> choosePaper() {
		HashMap<Key, Float> retValue = new HashMap<Key, Float>();
		// user dialog
		...
		if (paper != null) {
			...
			if (pageFormat.getOrientation() == PageFormat.PORTRAIT) {
				...
				// portrait
				//retValue.put(PDFTranscoder.KEY_WIDTH, (float) pageFormat.getPaper().getWidth());
			} else {
				logger.info("Orientation: Landscape");
				// landscape
				//retValue.put(PDFTranscoder.KEY_HEIGHT, (float) pageFormat.getPaper().getWidth());
				//retValue.put(PDFTranscoder.KEY_WIDTH, (float) pageFormat.getPaper().getHeight());
			}
		} else {
	...
	public boolean exportAsPdf(boolean nodeExport, MindMapNode selectedNode,
							   File chosenFile, HashMap<Key, Float> pTranscoderHints) throws Exception {
		 ...
		 */
		//pdfTranscoder.addTranscodingHint(PDFTranscoder.KEY_MAX_HEIGHT, 19200F);
		//pdfTranscoder.addTranscodingHint(PDFTranscoder.KEY_MAX_WIDTH, 19200F);
