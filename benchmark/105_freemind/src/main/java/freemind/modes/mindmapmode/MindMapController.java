...
public class MindMapController extends ControllerAdapter implements
	...
	public final MindMapController_Extracted2 mindMapController_Extracted2 = new MindMapController_Extracted2();
	public final MindMapController_Extracted1 mindMapController_Extracted1 = new MindMapController_Extracted1();
	...
	private Vector<AbstractAction> hookActions;
	// Mode mode;
	...
	public MindMapController(Mode mode) {
		...
		CompoundActionHandler compound = new CompoundActionHandler(this);
	...
	private void createStandardActions() {
		...
		cloudColor = new CloudColorAction(
				this);
	...
	private void createPatterns(List patternsList) {
	...
	protected void createNodeHookActions() {
		if (hookActions == null) {
			hookActions = new Vector<AbstractAction>();
			// HOOK TEST
	...
	public void nodeChanged(MindMapNode n) {
		...
		if (n == mapModule.getView().getSelected()) {
			toolbar.updateToolbar(n);
		}
	}

	public void onLostFocusNode(NodeView n) {
		super.onLostFocusNode(n);
		toolbar.updateToolbar(n.getModel());
	}
	...
	class DefaultMindMapNodeCreator implements NewNodeCreator {
	...
	public void updateMenus(StructuredMenuHolder holder) {
		...
		for (AbstractAction hookAction : hookActions) {
	...
	public JPopupMenu getPopupForModel(Object obj) {
		if (obj instanceof MindMapArrowLinkModel) {
			...
			links.addAll(getMindMapMapModel().getLinkRegistry().getAllLinks(
					link.getTarget()));
			for (Object o : links) {
				MindMapArrowLinkModel foreign_link = (MindMapArrowLinkModel) o;
	...
	// for it.
	class ExportToHTMLAction extends AbstractAction {
		MindMapController c;
	...
	class ExportBranchToHTMLAction extends AbstractAction {
		MindMapController c;
	...
	class ImportBranchAction extends AbstractAction {
		ImportBranchAction() {
	...
	private class MindMapFilter extends FileFilter {
		public boolean accept(File f) {
			...
			return extension
					.equals(FreeMindCommon.FREEMIND_FILE_EXTENSION_WITHOUT_DOT);
		}
	...
	public void blendNodeColor(MindMapNode node) {
		...
		Color nodeColor = blendNodeColor_Extracted2(node);
		setNodeColor(node,
	...
	public Color blendNodeColor_Extracted2(MindMapNode node) {
	...
	protected String getLinkByFileChooser(FileFilter fileFilter) {
		...
		relative = getLinkByFileChooser_Extracted4(null, chooser);
		return relative;
	}

	public String getLinkByFileChooser_Extracted4(String relative, FreeMindFileDialog chooser) {
		File input;
		int returnVal = chooser.showOpenDialog(getFrame().getContentPane());
		if (returnVal == JFileChooser.APPROVE_OPTION) {
	...
	public String[] getContent(String text, int pos) {
		if (pos <= 0) {
			return null;
		}
	 ...
	 *
	 */
	public void recursiveCallUpdateHooks(MindMapNode node,
	...
	public boolean extendSelection(MouseEvent e) {
		...
		boolean extend = extendSelection_Extracted3(e);
	...
	public boolean extendSelection_Extracted3(MouseEvent e) {
	...
	public void registerPlugin(MindMapControllerPlugin pPlugin) {
		mindMapController_Extracted2.getMPlugins().add(pPlugin);
	}

	public void deregisterPlugin(MindMapControllerPlugin pPlugin) {
		mindMapController_Extracted2.deregisterPlugin(pPlugin);
	}

	public Set getPlugins() {
		return mindMapController_Extracted2.getPlugins();
	}
	...
	public Transferable getClipboardContents() {
		return mindMapController_Extracted1.getClipboardContents(this);
	}
	...
	public void setClipboardContents(Transferable t) {
		mindMapController_Extracted1.setClipboardContents(t);
	}
